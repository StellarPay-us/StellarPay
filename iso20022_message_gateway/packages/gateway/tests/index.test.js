...
describe('POST /messages', () => {
  ...
  it('should return 500 if an error occurs during processing', async () => {
    ...
    messageService.validateXML.mockRejectedValue(new Error('Something went wrong'));
  ...
  it('should initialize the database correctly', (done) => {
    db.serialize(() => {
      db.all(`SELECT name FROM sqlite_master WHERE type='table' AND name IN ('messages', 'transactions', 'queue')`, (err, rows) => {
        expect(err).toBeNull();
        expect(rows.length).toBe(3); // Ensure all tables are created
        expect(rows.map(r => r.name)).toEqual(expect.arrayContaining(['messages', 'transactions', 'queue']));
        done();
      });
    });
  });
  ...
  it('should store and retrieve the correct values in the tables', (done) => {
    ...
    db.serialize(() => {
        ...
        db.run(
            ...
            VALUES (?, ?, ?, ?, ?, ?)`,
            [message.msg_id, message.cre_dt_tm, message.nb_of_txs, message.ctrl_sum, message.initg_pty_name, message.initg_pty_org_id],
            function (err) {
                expect(err).toBeNull();
                ...
                db.run(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                    [transaction.end_to_end_id, transaction.instd_amt, transaction.instd_amt_currency, transaction.xchg_rate_inf_unit_ccy, transaction.xchg_rate_inf_xchg_rate, transaction.cdtr_agt_bicfi, transaction.cdtr_acct_iban, messageId],
                    function (err) {
                        expect(err).toBeNull();
                        ...
                        db.run(
                            ...
                            function (err) {
                                ...
                                db.get(`SELECT * FROM messages WHERE msg_id = ?`, [message.msg_id], (err, row) => {
                                    expect(err).toBeNull();
                                    ...
                                    expect(row.initg_pty_org_id).toBe(message.initg_pty_org_id);

                                    // Verify that the transaction was inserted correctly
                                    db.get(`SELECT * FROM transactions WHERE message_id = ?`, [messageId], (err, row) => {
                                        expect(err).toBeNull();
                                        expect(row).toBeDefined();
                                        expect(row.end_to_end_id).toBe(transaction.end_to_end_id);
                                        expect(row.instd_amt).toBe(transaction.instd_amt);
                                        expect(row.instd_amt_currency).toBe(transaction.instd_amt_currency);
                                        expect(row.xchg_rate_inf_unit_ccy).toBe(transaction.xchg_rate_inf_unit_ccy);
                                        expect(row.xchg_rate_inf_xchg_rate).toBe(transaction.xchg_rate_inf_xchg_rate);
                                        expect(row.cdtr_agt_bicfi).toBe(transaction.cdtr_agt_bicfi);
                                        expect(row.cdtr_acct_iban).toBe(transaction.cdtr_acct_iban);

                                        // Verify that the queue record was inserted correctly
                                        db.get(`SELECT * FROM queue WHERE message_id = ?`, [messageId], (err, row) => {
                                            expect(err).toBeNull();
                                            expect(row).toBeDefined();
